...
function validateCpf(cpf) {
  ...
  if (cpf.length !== 11 || /^(.)\1+$/.test(cpf)) {
    return false; // CPF deve ter 11 dígitos e não pode ser composto por dígitos repetidos
  }
  ...
  if (resto === 10 || resto === parseInt(cpf.charAt(9))) {
    // Validação do primeiro dígito verificador passou, continue com o segundo
    soma = 0;
    for (let i = 0; i < 10; i++) {
